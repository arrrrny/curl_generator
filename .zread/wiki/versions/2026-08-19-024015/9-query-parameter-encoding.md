Query parameters are essential for filtering, pagination, and passing data in HTTP requests. This page explains how `curl_generator` handles query parameter encoding, the two approaches available, and important considerations for developers working with special characters.

## Two Approaches to Query Parameters

The library supports two distinct methods for adding query parameters to your curl commands, each with different trade-offs.

### Method 1: Inline in URL String

The simplest approach is to include query parameters directly in the URL string. This gives you complete control over the format and encoding.
...
### Method 2: Using the queryParams Parameter

The library provides a dedicated `queryParams` parameter that accepts a `Map<String, String>`. This approach is more structured and easier to maintain programmatically.
...
## How the queryParams Parameter Works
...
Now that you understand query parameter encoding, explore these related topics:

- **[HTTP Method Handling](8-http-method-handling)** — How different HTTP methods interact with query parameters
- **[Header Management and Content-Type Auto-Detection](10-header-management-and-content-type-auto-detection)** — Adding headers to your requests
- **[Body Serialization Strategies](11-body-serialization-strategies)** — Combining query parameters with request bodies
- **[HTTPS vs HTTP Behavior and --insecure Flag](12-https-vs-http-behavior-and-insecure-flag)** — Protocol-specific behaviors

## Quick Reference

| Scenario | Recommended Approach |
|----------|---------------------|
| Simple ASCII parameters | `queryParams` map |
| Parameters with special characters | Pre-encoded URL string |
| Dynamic parameter construction | `queryParams` with manual encoding |
| Complex URL with existing parameters | Pre-built URL string |
| Maximum control | `Uri` class + library |