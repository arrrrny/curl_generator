**curl_generator** is a lightweight Dart package that generates ready-to-use `curl` command strings from structured Dart objects — URLs, headers, query parameters, and request bodies. Instead of manually constructing shell commands, you pass your request data to a single static method and receive a properly formatted curl string you can paste directly into a terminal or share with teammates.

This page covers the absolute minimum you need to start generating curl commands in under five minutes. For a complete architectural understanding, see [Overview](1-overview); for deeper API exploration, continue to [Basic Usage Examples](4-basic-usage-examples).
...
Notice two things: the `--request POST` flag is prepended (because the method is not `GET`), and the `Content-Type` header is added automatically since one wasn't explicitly provided in your headers map. [Sources: lib/src/curl.dart#L70-L75](lib/src/curl.dart#L70-L75) [Sources: lib/src/curl.dart#L104-L143](lib/src/curl.dart#L104-L143)
...
Sources: [lib/src/curl.dart#L49-L67](lib/src/curl.dart#L49-L67) [Sources: lib/src/curl.dart#L104-L143](lib/src/curl.dart#L104-L143)
...
1. **[Basic Usage Examples](4-basic-usage-examples)** — More real-world patterns including `PUT`, `DELETE`, and nested body structures.
2. **[Installation](3-installation)** — Detailed installation for different project setups.
3. **[The Curl Class and Its API Surface](6-the-curl-class-and-its-api-surface)** — Deep dive into every method and field in the `Curl` class.
4. **[Header Management and Content-Type Auto-Detection](10-header-management-and-content-type-auto-detection)** — Understand how headers and `Content-Type` are resolved.
5. **[HTTPS vs HTTP Behavior and --insecure Flag](12-https-vs-http-behavior-and-insecure-flag)** — Full explanation of the security flag logic.